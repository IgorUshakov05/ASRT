...
Приложение реализует сервис для работы с расшифровками (транскриптами) встреч из Fireflies.ai.  
Основной функционал:

- Получение по ID или ссылке встречи её метаданных (названия, ссылки) через Fireflies.ai API (GraphQL).  
...
- Возвращение пользователю ссылки на созданный Google Sheets файл.
...
Если нужно, могу помочь с примерами использования или документацией по этим библиотекам.
...
- \`url\` — ссылка на встречу Fireflies.ai, например:  
  \`https://fireflies.ai/view/meeting:12345abcdef\`

### 4.2 Логика работы

1. Извлечение ID встречи из URL (парсинг по разделителю \`:\`).  
2. Получение метаданных встречи через GraphQL Fireflies.ai:  
   - Если встреча не найдена — возвращается ошибка.  
...
async function getTranscriptById(id) {
  ...
  try {
    ...
    return {
      success: false,
      message:
        "Текущий тариф не позволяет получить доступ к этой транскрипции",
    };
  }
}
\`\`\`
...
- Права сервисного аккаунта Google должны включать доступ к нужному общему диску (shared drive).  
- Fireflies.ai API имеет ограничения по тарифу — ошибки с текстом "Too many requests" или "subscription level" нужно обрабатывать отдельно.  
- Текст внутри ячейки Google Sheets не поддерживает частичное форматирование (например, жирность отдельных слов).  
- Парсинг транскрипта использует эмодзи-маркеры для разделения на темы, при необходимости адаптируется.

---
Тестовое задание для СибТехноСтрой
`;
